...
@master_only
def make_exp_dirs(opt): # Make dirs for experiments.
    path_opt = opt['path'].copy()
    
    # 如果上一次已经保存有了model,则重命名
    ...
    #根据训练模型创建experiment路径
    if opt['is_train']:
        experiments_root = path_opt.pop('experiments_root')
        if not opt['rename_flag']: # 删除原文件夹
            if os.path.exists(experiments_root):
                rmtree(experiments_root)
        net_type = opt['network_g']['type']
        mkdir_and_rename2(experiments_root, opt["rename_flag"])
        shutil.copy(opt['opt_path'], f'{experiments_root}/config.yml')
        shutil.copy(f'basicsr/models/archs/{net_type}_arch.py', f'{experiments_root}/{net_type}_arch.py')
    else:
        mkdir_and_rename(path_opt.pop('results_root'))
    
    # 创建path_opt 中的其他路径
    for key, path in path_opt.items():
        if ('weight' not in key) and ('strict_load' not in key) and  ('resume' not in key):
            os.makedirs(path, exist_ok=True)
